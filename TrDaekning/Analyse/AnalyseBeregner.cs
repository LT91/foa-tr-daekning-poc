...
namespace TrDaekning.Analyse
{
    internal class AnalyseBeregner
    {
        ...
        {
            const double kmPrGrad = 111.0;

            // Omregn forskel i breddegrad til kilometer
            double forskelBreddegradKm = (breddegrad2 - breddegrad1) * kmPrGrad;

            // Omregn forskel i længdegrad til kilometer (justeret med cos, fordi "km pr grad" afhænger af breddegrad)
            double gennemsnitsBreddegrad = (breddegrad1 + breddegrad2) / 2.0;
            double forskelLaengdegradKm =
                (laengdegrad2 - laengdegrad1) * kmPrGrad * Math.Cos(GraderTilRadianer(gennemsnitsBreddegrad));

            // Pythagoras: afstand = sqrt(x^2 + y^2)
            return Math.Sqrt(forskelBreddegradKm * forskelBreddegradKm +
                             forskelLaengdegradKm * forskelLaengdegradKm);
        }
        ...
        {
            // TR på arbejdspladsen (uanset overenskomst)
